...
const Dither = ({
  ...
  const onDraw = useEffectEvent(() => {
    ...
    // Source selection remains the same based on mediaType
    ...
    if (ditherMode !== "none" || hasColorAdj) {
  ...
  }, [mediaType, source]); // Dependent on unified 'source'

  return (
    <>
      <video
        ref={videoRef}
        src={mediaType === "video" ? source : undefined} // Only set src if it's a video
